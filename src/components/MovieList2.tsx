...
const MovieList = ({ data }: any) => {
  ...
  return (
    <div className="flex flex-wrap justify-start items-stretch text-white min-h-screen bg-[#10141e font-[1em]">
      {data?.map((item: any) => {
        if (!item) return null;
        return (
          <div
          ...
          >
            ...
            >
              ...
              />
              <div className="bg-white text-black flex-grow rounded-b-lg p-2 relative pt-4 min-h-[92px]">
                <span className="font-bold text-[1em] line-clamp-2">
                  {item?.title || item?.name}
                </span>
                <div className={`absolute -top-5 left-3 rounded-full  `}>
                  <RatingCircle voteAverage={item?.vote_average}></RatingCircle>
                </div>
                <div className="text-gray-500 pb-1 font-normal">
